...
export async function fetchJSON(url, { headers = {}, timeoutMs = 12000, method = 'GET', body } = {}) {
...
// Optional lock: if APP_PASSCODE is set on the server, every request must
// carry the same passcode (the app asks for it once and remembers it).
// This keeps strangers from using up your free data and AI limits.
export function guard(request) {
  const expected = process.env.APP_PASSCODE;
  if (!expected) return;
  if (request.headers.get('x-passcode') !== expected) {
    throw Object.assign(new Error('Passcode needed'), { code: 'locked' });
  }
}
...
export function fail(err) {
  if (err.code === 'locked') {
    return Response.json({ error: 'locked' }, { status: 401, headers: noStore });
  }
