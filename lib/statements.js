...
export function buildStatements(companyFacts) {
  ...
  const build = (basis, count, labelFor, growthLag) => {
    ...
    rows.grossProfit = rows.grossProfit.map((v, i) =>
      v ?? (rows.revenue[i] != null && rows.costOfRevenue[i] != null ? rows.revenue[i] - rows.costOfRevenue[i] : null));
    rows.freeCashFlow = ends.map((_, i) =>
      rows.operatingCashFlow[i] != null && rows.capex[i] != null ? rows.operatingCashFlow[i] - rows.capex[i] : null);
    ...
    return {
      ...
      rows,
      ratios: {
        grossMargin: ends.map((_, i) => pct(rows.grossProfit[i], rows.revenue[i])),
        operatingMargin: ends.map((_, i) => pct(rows.operatingIncome[i], rows.revenue[i])),
        netMargin: ends.map((_, i) => pct(rows.netIncome[i], rows.revenue[i])),
        fcfMargin: ends.map((_, i) => pct(rows.freeCashFlow[i], rows.revenue[i])),
      },
      growth: {
