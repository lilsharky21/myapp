  ...
  if (d.status === 'ok') {
    ...
    body = `
      ...
      <ol class="setup-steps">
        <li>In Terminal, run:${commandBox(command)}</li>
        <li>Click the llama icon in the menu bar → <strong>Quit Ollama</strong>, then open Ollama again.</li>
    ...
    body = `
      ...
      <ol class="setup-steps">
        ...
        ${browserTip}
        <li>Make sure this app's address is allowed (run this in Terminal, then quit and reopen Ollama):${commandBox(command)}</li>
      </ol>
      ${recheck}`;
  }
  return `<div class="setup-help diagnosis">${body}${wrongAddress}
    <p class="muted-line small-print">Mac restarted? Run the Terminal command again and reopen Ollama. The setting doesn't survive a restart.</p></div>`;
}
